...
async function getSelectedItineraryAndRequestPaxData(req, res) {
  ...
  const [queryResultOutbound] = await db.query(queryForFlightDetails, [
  ...
  if (typeOfTrip == "round-trip") {
    const [queryResultInbound] = await db.query(queryForFlightDetails, [
...
module.exports = {
  getSelectedItineraryAndRequestPaxData: getSelectedItineraryAndRequestPaxData,
};
